...
//    we made custom saver for TaskInfo
    ...
    AlertDialog(
        onDismissRequest = { onEvent(ToDoEvents.HideEditDialog) },
        confirmButton = {
            Button(
                onClick = {
                    ...
                    if (task.title.isNotEmpty() && task.description.isNotEmpty()) {
                        ...
                        val updatedTask = checkedList.find{ it.key==editTask.key }
                        updatedTask?.let {
                            val updatedIndex = checkedList.indexOf(updatedTask)
                            onEvent(ToDoEvents.updateCheckList(updatedIndex, task))
                        }
                        onEvent(ToDoEvents.AddTask(task))
                        onEvent(ToDoEvents.HideEditDialog)
